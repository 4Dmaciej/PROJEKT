const form = document.getElementById("siema");
...
function validateTytul(kname) {
	const isNotEmpty = kname.length > 0;
	const isNotBig = kname.length <= 128;
...
function validateISBN(iname) {
	const isISBN = /^(?=(?:\D*\d){10}(?:(?:\D*\d){3})?$)[\d-]+$/gm.test(iname);
...
function validateIloscEgzemplarzy(jname) {
	const isNotEmpty = jname.length > 0;
